...
export default function SuppliersList() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetchSuppliers({ page: 0, page_size: 50 });
      setData(res.items || []);
    } finally {
      setLoading(false);
    }
  ...
  const openModal = (record) => {
    ...
    if (record) {
      ...
      form.setFieldsValue({ status: 1, supplier_type: 1 });
    }
  };
  ...
  const handleOk = async () => {
    ...
    if (current) {
      await updateSupplier(current.id, values);
  ...
  const columns = [
    ...
    { title: "等级", dataIndex: "supplier_level" },
  ...
  return (
    ...
    >
      ...
      <Modal
        open={modalOpen}
        title={current ? "编辑供应商" : "新增供应商"}
        onOk={handleOk}
        onCancel={() => setModalOpen(false)}
        destroyOnClose
      >
        <Form layout="vertical" form={form}>
          <Form.Item name="supplier_code" label="编码" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          ...
          <Form.Item name="supplier_level" label="等级">
            <Select
              allowClear
              options={[
                { value: "A", label: "A" },
