...
const statusMap = {
  1: { text: "待退料", color: "default" },
  2: { text: "部分退料", color: "gold" },
...
export default function ReturnOrderList() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetchReturnOrders({ page: 0, page_size: 50 });
      setData(res.items || []);
    } finally {
      setLoading(false);
    }
  ...
  const openNew = () => {
    ...
    form.setFieldsValue({
      return_type: 1,
      plan_return_date: dayjs()
    });
    setModalOpen(true);
  };

  const openEdit = async (record) => {
    const full = await fetchReturnOrder(record.id);
    setCurrentId(record.id);
    form.setFieldsValue({
      return_no: full.header.return_no,
      ...
      return_type: full.header.return_type,
      plan_return_date: full.header.plan_return_date ? dayjs(full.header.plan_return_date) : null,
  ...
  const handleOk = async () => {
    ...
    const payload = {
      ...header,
      plan_return_date: header.plan_return_date?.format("YYYY-MM-DD") || null,
      details
    };
    try {
      if (currentId) {
        await updateReturnOrder(currentId, payload);
  ...
  const handleDelete = async (record) => {
    await deleteReturnOrder(record.id);
    message.success("已删除");
    load();
  };

  const columns = [
    { title: "退料单号", dataIndex: "return_no" },
    { title: "生产订单ID", dataIndex: "production_order_id" },
    { title: "仓库ID", dataIndex: "warehouse_id" },
    { title: "工单ID", dataIndex: "work_order_id" },
    { title: "计划日期", dataIndex: "plan_return_date" },
    { title: "实际日期", dataIndex: "actual_return_date" },
    {
      title: "总数量",
    ...
    >
      ...
      >
        <Form layout="vertical" form={form}>
          <Form.Item name="return_no" label="退料单号" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item
          ...
          <Form.Item name="return_type" label="退料类型" rules={[{ required: true }]}>
            <InputNumber min={1} max={3} style={{ width: "100%" }} />
          </Form.Item>
          <Form.Item name="plan_return_date" label="计划退料日期">
            <DatePicker />
          </Form.Item>
